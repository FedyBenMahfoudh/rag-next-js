...
import {addMessageParams, Conversation, Message, messageHistory} from "@/types";


export const createNewConversation = async (conversationName: string,userId:string) : Promise<Conversation>  => {
    const db = createClient();
    const { data , error} = await db.from("conversations").insert([{title : conversationName, user_id : userId}]).select();
...
export const deleteConversation = async (conversationId:string) :Promise<Conversation> => {
    const db = createClient();
    const { data, error } = await db.from("conversations").delete().eq('id', conversationId).select();
...
export const getConversationById = async (conversationId:string) : Promise<Conversation> => {
    const db = createClient();
    const { data, error } = await db.from("conversations").select("*").eq('id',conversationId).limit(1);
    if (error) {
        throw new Error("Could not get conversations");
    }
    // @ts-ignore
...
export const updateConversation = async (conversationId:string) : Promise<Conversation> => {
    const db = createClient();
